...
Among others, this information may be the structure of the environment (mapping or object tracking), the movement of the system to which is 
...
regard, from approaches based on geometrical constraints using sparse feature detection \cite{mur2015orb, ferrera2021ov} to methods leveraging photometric consistency
...
These algorithms aim at generalizing to any kind of environment and have their respective strengths and weaknesses \cite{fuentes2015visual}. 
...
In this chapter, we present two measurement models based on available software, that enable the implementation of this paradigm. The first one 
...
Both systems lack ways to estimate the uncertainty of their output and we, therefore, propose methods for the computation of their measurements outputs covariances.

In both models, we assume that we have a calibrated pinhole camera and that the images have been corrected for distortions. The camera reference frame $C$ has its origin at 
...
In this section, we will derive a general 6D relative transformation factor between a \keyframe\ and a object, whose poses in world frame are respectively $\T{W}{B} \in \SE(3)$ and $\T{W}{O} \in \SE(3)$. 
We denote by $\T{a}{b} \in \SE(3)$ 6D transformations transforming a 3D point $\posi{b}{} \in \Reals^3$ expressed in frame $b$ to its expression in frame $a$ $\posi{a}{}=\T{a}{b}\posi{b}{} \in \Reals^3$.
...
$\T{W}{O} = \T{W}{B}\T{B}{C}\T{C}{O}$ where W and B correspond to the world and body frames. $\T{B}{C}$ corresponds to the extrinsic pose  of the camera.
Given measurement $\Tm{C}{O}$, this relation can be turned into a residual relating 
...
\end{align}
%
where here $\Log_{\SE(3)}$ denotes the log map on $\SE(3)$ \cite{sola2018micro}.

Depending on the nature of the problem, we can fix some of the variables of the residual. In \tabRef{tab:res_var_fix}, various estimation problems are modeled depending on which variable is fixed.
...
This factor is general enough to be used in two applications that we successively describe, based on \apriltag\ fiducial markers and CosyPose. For both, we explain
...
Urban \cite[Eq. (23)]{urban2016mlpnp} uses a careful parameterization of the homogeneous coordinates associated with 2D features to develop
a maximum likelihood PnP algorithm and also proposes a covariance model based on the problem Hessian.  However, the particular choice of variable 
...
recover the pose covariance $\Cov_{\T{}{}}$. A simple model of pixel noise is to assume isotropic Gaussian noise on the pixels with variance $\sigma_x^2$. If we stack four pixel (the four tag corners) 
...
\end{equation}
%
where w denotes the dependency on the width of the marker. This "pnp" function implementation depends on the specificities of the PnP algorithm used and  
...
Regarding the transformation, we will consider it to be an element of the composite Lie group $\left<\Reals(3)\times \SO(3)\right>$ as it is done in our solver. 
The expressions of those functions are, therefore, expressed as:
...
Since $\Cov_{\bfx}$ and $\bfJ$ are known, this equation must be inverted in order to recover the needed covariance $\Cov_{\T{}{}}$. $\bfJ$ being non square and full column rank for four 
...
We simulate a realistic scenario using calibration data from a laboratory RealSense camera, 15 cm tags and $\sigma_x = 2~\text{pixels}$. 
\figRef{fig:apriltag_cov_P} and \figRef{fig:apriltag_cov_O} show how the uncertainty on position and orientation relate to different factors.
...
\begin{figure}[h]
    ...
    \begin{subfigure}{.50\linewidth}
        ...
        \caption{Relative position covariances, 15 cm tags \label{fig:apriltag_cov_P_015}}
    \end{subfigure}%
    \hfill
    \begin{subfigure}{.50\linewidth}
        \centering
        \includegraphics[width=\textwidth]{figures/apriltag_cov_P_030.png}
        \caption{Relative position covariances, 30 cm tags \label{fig:apriltag_cov_P_030}}
    \end{subfigure}%
...
orientation as in \figRef{fig:apriltag_cov_O_10deg}. On the contrary, tags with higher relative orientation in \figRef{fig:apriltag_cov_O_10deg} display a much lower rotational uncertainty.
...
\begin{figure}[h]
    ...
    \begin{subfigure}{.50\linewidth}
        ...
        \caption{Relative orientation covariances, 10° orientation \label{fig:apriltag_cov_O_10deg}}
    \end{subfigure}%
    \hfill
    \begin{subfigure}{.50\linewidth}
        \centering
        \includegraphics[width=\textwidth]{figures/apriltag_cov_O_45deg.png}
        \caption{Relative orientation covariances, 45° orientation \label{fig:apriltag_cov_O_45deg}}
    \end{subfigure}%
    ...
    \apriltags\ placed in front of the camera at different distances but identical orientations. Covariances are represented in orange, centered at the corresponding tag 
    position.
    In (\subref{fig:apriltag_cov_O_10deg}), tags are oriented to be nearly fronto parallel to the camera (10 degrees orientation), resulting in high orientation uncertainty due to the ambiguous perspective. 
    (\subref{fig:apriltag_cov_O_45deg}) shows tags with the same positions but 45 degree orientations with respect to the camera, which results in lesser uncertainty.}
...
10 cm to the right, and the third (rose) 10 cm behind. We can see that the difference of appearance due to a depth-wise movement is much smaller, which 
...
actually uses depth measurements and is dominated by a purely RGB, deep-learning-based model \cite{haugaard2021surfemb}, though \cite{konig2020hybrid} 
...
while object tracking and mapping is handled by the SLAM framework. 
...
The single-view procedure for pose estimation of CosyPose is an improvement over the one proposed in DeepIm \cite{deepim_2019}. The general idea is to 
...
that best aligns the transformed points. In practice, for continuous symmetries  that are rotations around an axis (\eg for a texture-less cylinder), 
...
We then perform polynomial regression implemented with Scikit Learn \cite{scikit-learn}. A simple linear regression leads to a high root-mean-square error (RMSE) 
...
This empirical model has the drawback that it needs to be trained (ideally with real data)
...
A more quantitative evaluation can be deduced from \tabRef{tab:empirical_models}. The translation error seems to be captured pretty well,
...
the scale is different: the stair is $1m \times 0.3m \times 0.07m$ whereas a T-LESS object is never wider than $10 cm$. 
...
\ie mapping while directly extracting the location of an object of interest. With
