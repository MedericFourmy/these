...
At each reception of sensor data $\tilde\bfz_k$ at $t_k$, we integrate during $\delta t$ to obtain the delta corresponding to a single data sample
%
\begin{align}
\bm\delta_k = f(\tilde\bfz_k, \ol\bfb_i, \delta t)~, 
...
where $\bfb_i$ is the current bias value,  $\hat\bfDelta_{im}=\bfx_k\boxminus\bfx_i$ is the expected delta between KFs, and $\{\op,\om\}$ are the composite
...
The residual clearly depends on the KF states $\bfx_i,\bfx_m$ and the bias $\bfb_i$. It has an associated covariance  $\bfSigma^\Delta_{im}$.


\subsection{IMU pre-integration factor $\bfr^{I}$}
...
\small
\begin{split}
\COM_{k} &= \COM_{k-1} + \COMd_{k-1} \delta t+\frac{1}{2} \bfg \delta t^2 + \frac{1}{2m} \Rot{}{k-1} \sum_L \Rot{B}{Lk} (\forcem{}{Lk} - \noise_{f}) \delta t^2
\\
\COMd_{k}&= \COMd_{k-1} + \bfg \delta t + \frac{1}{m} \Rot{}{k} \sum_L\Rot{B}{Lk} (\forcem{}{Lk} - \noise_{f}) \delta t 
\\
\AM_{k} &= \AM_{k-1} +\Rot{}{k} \sum_L (\posi{B}{Lk}  - \posim{B}{Ck} +  \bias_{ck} + \noise_{c}) \times \Rot{B}{Lk}(\forcem{}{Lk} - \noise_{f}) 
 \delta t
\end{split}
...
%
\begin{equation}
\bfz_k = \left[ \posim{B}{Ck}, \angvelm{}{k}, \left[\forcem{}{Lk}, \posim{B}{Lk}, \Rotm{B}{Lk} \right]_{L=1\dots4}\right]
\end{equation}
...
\begin{equation}
\small
    \bm\delta_{k}(\bfz_k, \bfb_i, \dt) =
    \begin{bmatrix}
    \frac{1}{2m} \sum_l \Rotm{B}{Lk} \forcem{}{Lk} \delta t^2
    \\
    \frac{1}{m} \sum_l \Rotm{B}{Lk} \forcem{}{Lk} \delta t 
    \\
    (\sum_l \posim{B}{Lk} - (\posim{B}{Ck} - \bias_c) \times (\Rotm{B}{Lk} \forcem{}{Lk} ))\delta t
    \\
    \Exp((\angvelm{}{k} - \bias_{\omega})\delta t)
    \end{bmatrix}
\normalsize
\end{equation}
%
...
=
\begin{bmatrix}
	{\Rot{}{i}}\tr (\COM_k - \COM_i - \COMd_i \Delta t_{ik})
	\\
	{\Rot{}{i}}\tr (\COMd_{k} - \COMd_{i} - \bfg \Delta t_{ik})
	\\
	{\Rot{}{i}}\tr (\AM_{k} - \AM_{i})
	\\
{\Rot{}{i}}\tr \Rot{}{k}
...
\begin{equation}
	...
	\begin{bmatrix}
	\COM_i + \COMd_i \Delta t_{ik} + \Rot{}{i} \Delta \COM_{ik} + \frac{1}{2} \bfg \Delta t_{ik}^2
	\\
	\COMd_i + \Rot{}{i} \Delta \COMd_{ik} + g \Delta t_{ik}
	\\
