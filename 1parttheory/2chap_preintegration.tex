...
%
\begin{align}
    \D &= \Exp(\bftau) \te \exp(\bftau\hhat) = \begin{bsmallmatrix}
    \Exp(\bth) & \bfQ\bfupsilon & \bfQ\bfrho+\bfP\bfupsilon\Dt \\
    \bf0 & 1 & \Dt \\
    \bf0 & 0 & 1
    \end{bsmallmatrix} %\in\cD 
    \label{equ:exp}
    \\
    \bftau &= \Log(\D) \te \log(\D)\vvee = \begin{bsmallmatrix}
...
In this last equation, the general $\oplus$ operator is simply the traditional Lie group "minus" operator defined in [cite sola eq ...] applied
...
\begin{align}
    ...
    \bf0 \\ \bfa \\ \bw \\ 1
    \end{bsmallmatrix}\dt\right) \approx \begin{bsmallmatrix}
...
%
\begin{align}
    \D_{ik} = \prod_{j=i+1}^k \Exp\left(\begin{bsmallmatrix}
