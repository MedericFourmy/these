...
where then override our delta notations as $\hat\bfDelta_{im}(\bfx^i, \bfx^m) = \bfDelta_{im}(\bias^i)$,
emphasizing the fact that the right hand side term does not depend on $\bfx_i$ anymore. 

However, the bias terms in \eqRef{eq:IMUPreintij} still enforce the repeated reintegration of the measurements buffer. 
A better approach is to define $\ol\bfDelta_{im}$ as the integration right-hand side integration using biases estimates at the time where we 
begin to integrate, which we note $\biasbar^i$. Then, we can approximate changes due to the newly estimated bias $\bias_i$ using a first order approximation:
% $\bfDelta_{im}(\bias^i) = \bar{\bfDelta}_{\cZ_{im}} \oplus (\bfJ^{\bfDelta_{im}}_{\bias_i} (\bias^i - \biasbar^i))$.

\begin{align}
    \bfDelta_{im}(\bf^i) = \ol\bfDelta_{im} \oplus \mjac{\bfDelta_{im}}{\bfb_i}(\bfb_i-\ol\bfb_i).
...
The above equations are not directly useful for the implementation of pre-integration. We need first to derive a recursive formulation 
