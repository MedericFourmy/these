...
are defined between \keyframes\ selected at a relatively low frequency. Ones needs to integrate measurements from these high-rate sensors between \keyframes\,
...
Let's consider the estimation of a robot base state comprised of its pose and velocity in the world frame. 
...
Other sensors can help estimate IMU biases $\bias \triangleq [\bias_{\acc{}{}}, \bias_{\angvel{}{}}]$ so we include them in the estimator state.
...
% $e(\bfx^i, \bfx^j) = \bfDelta_{\bfx}(\bfx^i, \bfx^j)  \ominus  \bfDelta_{im}(\bias^i, \bfx^i)$
%   where 
%
\begin{align}
...
Both quantities are recomputed at each evaluation of the residual by the nonlinear solver. $\hat\bfDelta_{im}(\bfx^i, \bfx^m)$ is cheap to compute.
However, $\bfDelta_{im}(\bias^i, \bfx^i)$ requires to reintegrate the whole buffer of measurements, due to the dependency on velocity, 
orientation and biases at timestamp $i$. This is highly inefficient. 

Nevertheless, the terms of equation \eqRef{eq:IMUIntij} can be rearranged (proof in the annex \secRef{sec:forster_proof}) to give:
%
% WITH NOISE
% \begin{equation}
%     \begin{split}
...
\end{equation}
%
% where we define $\DR^{ik} \triangleq \Rot{}{i,T} \Rot{}{}^k$.  % NOT really needed to precise and would require \Dv^{ik} which is longer to write 
We then overwrites our delta notations as $\hat\bfDelta_{im}(\bfx^i, \bfx^m) = \bfDelta_{im}(\bias^i)$,
emphasizing the fact that the right hand side term does not depend on $\bfx_i$ anymore. However the bias terms in \eqRef{eq:IMUPreintij} still enforce the repeated 
reintegration of the measurements buffer. Instead, the right hand side can be integrated once, giving $\ol\bfDelta_{im}$ using biases estimates at the time where we 
begin to integrate them, which we note $\biasbar^i$. Then, we can approximate changes due to the newly estimated bias $\bias_i$ using a first order approximation:
% $\bfDelta_{im}(\bias^i) = \bar{\bfDelta}_{\cZ_{im}} \oplus (\bfJ^{\bfDelta_{im}}_{\bias_i} (\bias^i - \biasbar^i))$.

\begin{align}
    \bfDelta_{im} = \ol\bfDelta_{im} \oplus \mjac{\bfDelta_{im}}{\bfb_i}(\bfb_i-\ol\bfb_i).
