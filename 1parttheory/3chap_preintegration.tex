\chapter{High-rate sensors data pre-integration Preintegrated}
...
Now, these equations relate variables  between two state variables sampled at IMU frequency. To include these measurements in our smoothing estimator,
...
Fortunately, the terms of equation \eqRef{eq:IMUIntij} can be rearranged (proof in the annex \secRef{sec:forster_proof}) to give:
%
% WITH NOISE
% \begin{equation}
%     \begin{split}
...
We then overwrite our delta notations as $\hat\bfDelta_{im}(\bfx^i, \bfx^m) = \bfDelta_{im}(\bias^i)$,
emphasizing the fact that the right hand side term does not depend on $\bfx_i$ anymore. 

However, the bias terms in \eqRef{eq:IMUPreintij} still enforce the repeated 
