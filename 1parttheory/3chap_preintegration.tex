...
\begin{equation}
    ...
    = \bfDelta_{im}(\bias^i, \bfx^i) \ominus \hat\bfDelta_{im}(\bfx^i, \bfx^m) =
    \begin{bmatrix}
    \Dp_{im} - \hat\Dp_{im} \\ 
    \Dv_{im} - \hat\Dv_{im} \\ 
...
$\hat\bfDelta_{im}$ only depends on state variables and, thus, is cheap to compute. It corresponds to the "expected" motion of the system, given the current state estimates. 
$\bfDelta_{im}$ is the motion computed from the integration of (very many) IMU measurements during $\Dt_{im}$. However, since we "naively" integrated
...
To solve this problem, we need to re-define the deltas so that $\bfDelta_{im}$ is independent of the estimated states, that is, only dependent on the measured data. This new definition reads \cite{lupton-09, forster2015imu} (proof in the annex \secRef{sec:forster_proof}):
%
\begin{align}
...
\end{align}
%
Let us now emphasize the fact that $\bfDelta_{im}$ as defined in \eqRef{eq:imu_delta} does not depend on $\bfx_i$, contrary to \eqRef{eq:imu_delta_naive}. 

But we are not over yet. The dependency on the bias variable $\bfb^i$ in \eqRef{eq:imu_delta} still enforces the repeated reintegration of the measurements buffer to get $\bfDelta_{im}$ each time the solver produces a new estimate of $\bfb^i$. 
However, because the variations in $\bias_i$ are small, a linearized approximation can be used. The IMU measurements can be pre-integrated using the prior bias estimation at time $t_i$, that we note $\ol\bfb_i$, to give the pre-integrated delta $\ol\bfDelta_{im}\triangleq\bfDelta_{im}(\ol\bfb_i)$.
...
We will now show how this formulation can be generalized to other high rate sensor by abstracting a recursive implementation of the pre-integration that takes advantage of the Lie-group structure of the geometry of the deltas.
...
The Delta Lie group used in the specific application has to be defined, by describing in particular its identity element $\bfDelta_{\cE}$, the composition law $\circ$, and the $\Exp$ and $\Log$ operators. Likewise, in order to deal with the uncertainty of the integrated data, Jacobians of these operators will need to be obtained. 

The motion data usually expresses the way these deltas change, and can therefore easily be put in the tangent space of the Deltas Lie group. In order to account for sensor self-calibration, this motion data is first corrected for using known calibration parameters, then retracted to the Lie group using the exponential map, then composed with the delta pre-integrated so far to obtain, recursively, a new value of the pre-integration.
...
Dependence on other small-varying parameters $\bfb$ such as sensor bias or other calibration parameters is linearized as $\bfDelta_{im}(\bfb)=\bfDelta_{im}(\ol\bfb) \op \bfJ^\bfDelta_\bfb(\bfb-\ol\bfb)$.
...
First, $\ol\bfDelta_{ii}$ is initialized to the null motion via the identity of the group, $\bfDelta_{\cE}$. 
...
In major cases, this function is split into the stages of data calibration $c()$ and retraction $\Exp()$,
%
\begin{align}
    \bm\delta_k = \Exp(c(\tilde\bfz_k, \ol\bfb_i) \dt),
\end{align}
%
...
The Jacobian of the pre-integrated delta with respect to biases, as well as the delta covariance, are also pre-integrated using the chain rule and standard covariance propagation,
%
\begin{align}
...
\end{align}
%
where $\mjac{\bm\delta_k}{\bfb_i}$, $\mjac{\bm\delta_k}{\bfz_k}$ are the Jacobians of \eqRef{eq:data2delta} and $\mjac{\bfDelta_{ik}}{\bfDelta_{ij}}$, $\mjac{\bfDelta_{ik}}{\bm\delta_k}$ the Jacobians of \eqRef{eq:deltaPlusDelta}, computed according to Lie theory \cite{sola2018micro}, to which we give a brief introduction in \secRef{sec:lie_groups}.
...
$\bfb_i$ is the current  value of the sensor's calibration parameters,  $\hat\bfDelta_{im}=\bfx^m\boxminus\bfx^i$ is the expected delta between KFs, and $\{\op,\om\}$ are the 
...
As for the IMU case, the calibration drift is modelled as the integration of a random walk. This drift is included as a factor in the factor graph by defining a residual error
...
The group composition law $\bfDelta^{ik}=\bfDelta^{ij}\circ\bm\delta^k$ in \eqRef{eq:deltaPlusDelta} is defined as
%
\begin{equation} 
...
Note that the function $f$ \eqRef{eq:delta_function}, integrating one step of motion data, is here decomposed in two operations: 
first the debiasing presented in \eqRef{preint_debiasing}, then the integration using the delta group exponential map \eqRef{eq:compact_exp}.
...
\subsubsection{IMU Lie group versus Forster's method}
