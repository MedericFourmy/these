...
Different Lie group structures can be chosen that satisfy the properties above. For the IMU, we explored the two variants: composite \cite{forster2015imu} and compact group 
(our paper \cite{fourmy2019absolute}). 
...
The definition of the deltas needs to satisfy the property that such deltas resulting from the integration of motion data must be independent on the initial state $\bfx^i$. 
The general rule for this stems from observing that an unbiased and unnoisy sensor measuring null data should produce null deltas. This means that the deltas would be the 
deviations from the state resulting from null data -- for example, a free-falling frame in  the case of an IMU.
...
\end{itemize}
%
Likewise, in order to deal with the uncertainty of the integrated data, Jacobians of these operators will need to be obtained.
...
This derives from the definition of the operator $\boxminus$ in \eqRef{eq:boxminus}, an example of which was detailed in \eqRef{eq:boxminus-forster}, 
the operator $\boxplus$ in \eqRef{eq:boxplus} being just its inverse.
...
\end{align}
%
The three constituents of such deltas, namely $\Dp$, $\Dv$ and $\DR$, are defined exactly as in the composite case above. Notice a fourth component 
$\Dt$ as being also part of the delta definition. Being computed in the same manner, the definition of the $\boxminus$ and $\boxplus$ operators is a trivial 
manner of composing the matrix deltas from \eqRef{eq:boxminus-forster}, yielding proper expressions for
%
\begin{align*}
...
\subsubsection{The case of IMU data leading to \texorpdfstring{$\bfv=0$}{bfv=0}}
\label{sec:IMU_case}
...
\begin{align}
    \D_{im}(\bfb_i) &= \ol\D_{im}\circ\Exp(\mjac{\D_{im}}{\bfb}(\bfb_i-\ol\bfb_i)) 
    ~.
\end{align}
