...
In cases where the calibration parameters are subject to drift, as it is the case for IMU biases \eqRef{eq:bias_imu_drift}, the calibration drift is modelled as the integration of a random walk. This drift is included as a factor in the factor graph by defining a residual error
%
\begin{equation}
    \bfe^{\bias}_{im}(\bias^i, \bias^m) = \bias^m - \bias^i
\end{equation}
