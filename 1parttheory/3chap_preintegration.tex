...
The delta covariance, as well as the Jacobian of the pre-integrated delta with respect to biases, are also pre-integrated using the chain rule and standard covariance propagation,
%
\begin{align}
...
a factor is created to which we pass $\ol\D_{im}$, $\ol\bfb_i$,  $\mjac{\D_{im}}{\bfb_i}$, and $\Cov_\Delta^{im}$, which are enough define the residual:
...
Let us come back to the IMU pre-integration problem, as stated by Forster \cite{lupton-09, forster2015imu}, defined in \secRef{sec:imu_preint_motivation}.
...
\subsubsection{IMU factor residual on compact Lie group}
