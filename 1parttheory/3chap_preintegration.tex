...
However, because the variations in $\bias_i$ are small, a linearized approximation can be used. The IMU measurements can be pre-integrated using the prior bias estimation at time $t_i$, that we note $\ol\bfb_i$, to give the pre-integrated delta $\ol\D_{im}\triangleq\D_{im}(\ol\bfb_i)$.
...
The successive $\ol\D_{ik}$, $\mjac{\D_{ik}}{\bfb_i}$, and $\Cov_\Delta^{ik}$ are kept in a buffers initialized at $t_i$. When a new \keyframe\ is added to the problem,
a factor is created to which we pass $\ol\D_{im}$, $\mjac{\D_{im}}{\bfb_i}$, and $\Cov_\Delta^{im}$, which are enough define the residual:
...
$\bfb_i$ is the current  value of the sensor's calibration parameters,  $\hat\D_{im}=\bfx^m\boxminus\bfx^i$ is the expected delta between KFs, and $\{\op,\om\}$ are the 
