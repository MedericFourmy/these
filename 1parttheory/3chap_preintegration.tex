...
The definition of the deltas needs to satisfy the property that such deltas resulting from the integration of motion data must be independent on the initial state $\bfx^i$. The general rule for this stems from observing that an unbiased and unnoisy sensor measuring null data should produce null deltas. This means that the deltas would be the deviations from the state resulting from null data -- for example, a free-falling frame in  the case of an IMU.
...
This derives in the definition of the operator $\boxminus$ in \eqRef{equ:boxminus} as what we detailed in \eqRef{equ:boxminus-forster}, the operator $\boxplus$ in \eqRef{equ:boxplus} being just its inverse.
...
Being computed in the same manner, the definition of the $\boxminus$ and $\boxplus$ operators is a trivial manner of composing the matrix deltas from \eqRef{equ:boxminus-forster}, yielding proper expressions for
%
\begin{align*}
