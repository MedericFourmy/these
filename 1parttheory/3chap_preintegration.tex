...
But we are not over yet. The dependency on the bias variable $\bfb^i$ in \eqRef{eq:imu_delta} still enforces the repeated reintegration of the measurements buffer 
...
In \chpRef{chp:underactuade_dynamics}, we also use this generalized approach for the the pre-integration of force-torque sensors for the centroidal estimation of legged robots 
...
be independent from the initial state $\bfx^i$. The general rule to achieve this stems from observing that an unbiased and unnoisy sensor measuring null data should 
...
In cases where the calibration parameters are subject to drift, the calibration drift is modelled as the integration of a random walk. This drift is included as a second factor 
...
$\boxplus$ in \eqRef{equ:boxplus} as being just its inverse.
...
Invariant Kalman Filter \cite{barrau2018invariant, hartley2020contact}. This methods tackles one of the issues of standard EKF: 
...
With this new formulation, Lupton noted that pre-integration of IMU measurements permitted to use measurements immediately and delay the estimation of the initial 
...
Barrau \cite{barrau2020mathematical} described a compact matrix Lie group for the propagation of pre-integration errors taking into account the earth rotation with aerospace
...
We show that permits to estimate the centroidal quantities of the robot as well making the kinematic bias on center of mass measurements observable. 
