...
Then, the residual $\bfe^{CK} \in \Reals^9$ is simply expressed as:
%
\begin{equation}
    \bfe^{CK}=
    \begin{bmatrix}
        (\Rot{}{}^T(\COM - \posi{}{}) + \bias_{c}) - \posim{B}{C}
        \\
        \Rot{}{}^T(\COMd - \vel{}{}) - \left[(\angvelm{}{} - \bias_{\omega}) \times (\posim{B}{C} -  \bias_{p}) + \velm{B}{C} \right]
        \\
        \Rot{}{}^T\AM - \left[\Inertia (\angvelm{}{} - \bias_{\omega}) + \AM_a \right]
    \end{bmatrix}
    \label{eq:CKFactor}
\end{equation}
...
forces applied on a legged robot in a smoothing estimator. We propose to integrate the underactuated dynamics \eqRef{NewtonEuler} using force torques measurements.
