...
\end{equation}
%
where is a Gaussian noise and $\bfb_{c}$ is a varying bias that we wish to estimate.
...
forces applied on a legged robot in a smoothing estimator. We propose to integrate the underactuated dynamics \eqRef{NewtonEuler} using force-torque measurements.
...
The definition of delta and $\boxminus$ operator in the force-torque pre-integration case is:

\begin{equation}
    \widehat\D^{im} = \bfx^m \boxminus \bfx_i =
    \begin{bmatrix}
...
% We now derive the group operations
...
Where the problems differ however is in the inclusion of the angular momentum (the Euler part of \eqRef{eq:NewtonEuler}). This equation introduces the dependence on 
