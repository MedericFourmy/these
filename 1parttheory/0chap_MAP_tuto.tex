...
following the approach by find the mode of the posterior distribution on the states. 
...
then the vector substraction needs to be replaced:
...
\end{equation}
%
where $[\cdot]\hhat$ is the skew-symmetric operator associated to the cross product. 
...
A crucial aspect of solving the MAP problem is the fact that the the likelihood function is factorizable. This represents the fact that the problem
...
are described as residuals weighted by their respective covariance matrices, representing the confidence we have on the sensor measurements. Factor Graphs can 
...
formulating new, efficient, and generalizable measurement models. This thesis mostly theoretically contribute to the direction of extending
the estimation model (ii) while we mostly rely on existing algorithms (i) to solve the subsequent problems.
In state estimation for robotics, a commonly used denomination is to refer to the solver implementation as the \textit{backend} while the measurement models are referred to as \textit{front-ends}.
