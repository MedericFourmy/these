...
This problem is best visually represented by a probabilistic graphical model known as the \textit{factor graph}, which will be described in \secRef{sec:factor_graphs}
...
to mitigate the shortcomings of the "pure" Gauss-Newton algorithm. Those algorithms are trade offs between accuracy, speed, and implementation complexity, that
suits better in different applications. It seems that, for robotics, Levenberg-Marquadt has proven to be a good trade off between the real-time and
...
be taken toward the conditioning of the problem Hessian \cite{jian2012generalized}.
...
For rotation matrices, the adjoint is simply $\text{Ad}_{\bfR}\SO(3)=\bfR$. Those operations are illustrated in \figRef{fig:rotation_exp_log}.
