...
The task of \textit{estimation} can then simply be stated as finding the robot state given these measurements. This intuition can be formalized
as finding the mode of the posterior distribution on the states conditioned by the measurements. 
...
$p(\cZ | \cX)$ is the measurement model, also called the \textit{likelihood} of the observation. 
...
The \textit{marginalized likelihood} $p(\cZ) = \int p(\cZ|\cX)p(\cX)d\cX $ can be thought of as a global normalization constant (\cite[Chapter 20]{koller2009probabilistic}) in case 
...
For example, in the context of robotics, recursive Monte Carlo sampling (\cite[Chapter 12]{koller2009probabilistic}) has been 
...
Another type of approximate inference, with scarcer applications in robotics until now, is variational inference (\cite[Chapter 11]{koller2009probabilistic}). 
...
$\Cov_i \in \Reals^{M_i \times M_i}$ are the covariances of the measurements' noise,
$\phi_i(\cX_{S_i})$ are the un-normalized measurement likelihoods called \textit{factors}, and 
%
\begin{equation*}
...
\begin{align}
    ...
    \\
    &= \argmin_{\cX} - \log p(\cX_0) \prod_{i=1}^{n} p(\bfz_i | \cX_{S_i})  &\text{\small Conditional independences} \label{eq:factorized_likelihood}
    \\
...
\begin{enumerate}
    ...
    \item Approximate the NLLS cost function around the current estimate as a quadratic function (see \eqRef{eq:gaussian_step_quadratic_eq} below).
...
%
\begin{equation}
    \cX^{MAP} = \argmin_{\cX} ||\bfr(\cX)||^2 
\end{equation}
%
where $\bfr$ is a vector of vertically stacked residuals (column vectors) and the cost function is the squared norm 
...
$\bfJ \in \Reals^{M \times N}$. We call this new function $\cF(\Delta \bfx) \approx ||\bfr(\cX + \Delta \bfx)||^2$, so that the linearized cost function writes:  
%
\begin{equation}
    \cF(\Delta \bfx) = ||\check{\bfr} + \bfJ \Delta \bfx||^2 
    = \check{\bfr}\tr\check{\bfr} +  2\check{\bfr}\tr \bfJ \Delta \bfx + \Delta \bfx\tr \bfJ\tr \bfJ \Delta \bfx.
    \label{eq:gaussian_step_quadratic_eq}
\end{equation}

$\cF(\Delta \bfx)$ is therefore a local parabolic approximation of the squared residual around the current estimate 
and $\bfH \triangleq \bfJ\tr \bfJ$ is the approximate Hessian of the squared residual with respect to the state variables 
\footnote{See \cite[Section 4.2.1]{sola2017course} for more details on the nature of the approximation.}. Note that by construction, $\bfH$ is always 
...
%
\begin{equation}
    \Delta \bfx_{L}^* = - \alpha (\bfH + \lambda \bfI)^{-1} \bfJ\tr \bfr
\end{equation}
%
...
%
\begin{equation}
    \Delta \bfx_{L}^* = - \alpha (\bfH + \lambda \text{diag}(\bfH))^{-1} \bfJ\tr \bfr.
\end{equation}
...
We define the composite manifold as the concatenation $\cM_c \triangleq \left<\cM^1, \dots, \cM^M \right>$. An element of this composite manifold is denoted by $\cX$.
We will also write a composite Lie group  equivalent of the retraction $\oplus$ and lift $\ominus$ operations. 
It consists in applying operations to individual elements of the manifold and its tangent space:
...
Our state variables are random variables approximated by multivariate Gaussian distributions, that can be summarized by their mean vector and covariance matrix.
These concepts need to be slightly modified to account for the manifold nature of some of these by defining Gaussian distributions on manifolds.
...
By a slight abuse of notation, we note that the manifold distribution is $\bfx \sim \Gaussian{\bar{\bfx}}{\Cov_{\bfx}}$.
...
\begin{equation}
    f~: \bfx \in \Reals^{n_x} \rightarrow \bfy \in \Reals^{n_y}, \quad \quad \Cov_{\bfy} \approx 
                \left.\frac{D f}{D \bfx} \right|_{\bfx}   \Cov_{\bfx}    \left.\frac{D f}{D \bfx}\right|_{\bfx}\tr
...
state variables that the residual depends on are non-zero. For instance, the Jacobian corresponding to the toy problem described in \figRef{fig:toy_problem} is:
\begin{equation}
    \bfJ=
    \begin{pmatrix}
        J^{0}_{\bfx_1} &   &     &     &     \\
...
at each step very efficient. Algorithms such as the Schur complement \cite[Section A.5.5]{boyd2004convex}, Cholesky factorization \cite[Section C.3]{boyd2004convex}, 
and QR factorization \cite[Section C.5]{boyd2004convex} were applied by different authors to the problem of SLAM and estimation. 
...
\begin{enumerate}
    \item Initialize the state estimate at an initial value $\check{\cX} := \cX^0$ \\
    $\cX$ lives on a composite manifold. Its error and increments are expressed in its local tangent space.
    ...
    may alter the linear system to be solved for increased performance.
...
represent visually those problems and their connectivity exactly translates to the sparsity of the linear problems appearing in the optimization algorithm.
...
A commonly used denomination in the context of robotics state estimation is to refer to the solver implementation as the \textit{backend} while the algorithms processing sensor information to create residuals are referred to as \textit{front-ends}.

The next chapters concentrate on a range of sensor modalities useful for legged robots:
