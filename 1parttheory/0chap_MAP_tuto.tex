...
In practice, the individual Jacobians \eqRef{eq:res_jacobian} are usually obtained by automatic differentiation, \eg using dual number \cite{ceres-solver} and $\bfJ^{+}$ is not constructed explicitly. Efficient linear system solvers based on the Cholesky
...
that suits better in different applications. It seems that, for robotics, Levenberg-Marquardt has proven to be a good trade off between the real-time and
...
the existence of inverse for each element, the adjoint linear map, and the possibility to interpolate between elements.
...
\begin{align}
    ...
    \text{Lift}~:&\quad\quad \bftau = \bfx_2 \ominus \bfx_1
\end{align}
...
We will also write composite equivalent of the retraction $\oplus$ and 
...
\begin{equation}
    \Cov_{\bfx} \triangleq \mathbb{E}[\bftau\bftau^{\top}] = \mathbb{E}[(\bfx \ominus \bar{\bfx})(\bfx \ominus \bar{\bfx})^{\top}] \in \Reals^{n \times n}
\end{equation}
%
...
Riemmanian geometry is enough to deal with optimization problems with variables living on smooth manifold. However, many of these state variables, in the context of robotics, also exhibit a group structure, which give them extra interesting properties.
...
%
\begin{equation}
    \SO(3) = \left\{\Rot{}{} \in \text{GL}(n,\Reals) | \Rot{}{}\Rot{}{}^{\top}=\Rot{}{}^{\top}\Rot{}{}=\bfI,~det(\Rot{}{})=1\right\}
\end{equation}
%
...
Finally, we have denoted by $\Exp$ the composition of the $hat$ operator ($\bftheta\hhat \triangleq [\bftheta]_{\times} \in \so(3)$) and the matrix exponential: $\Exp(\bftau) \triangleq \exp(\bftau\hhat)$. 
...
\begin{align}
    ...
    \prescript{1}{}{\bftheta} &= \Rot{}{2}\ominus \Rot{}{1} = \Log(\Rot{}{1}^{\top}\Rot{}{2}) 
\end{align}
...
Many specialized solvers \cite{grisetti2011g2o, kaess2012isam2, ila2017slam++} have been implemented to exploit this sparsity. A discussion of the particularities of factor graph-based NNLS solvers can be found in \cite{dellaert2017factor, sola2017course}.
...
problem of SLAM and estimation. Descriptions of these linear solvers can be found in \cite{sola2017course, dellaert2017factor}.
...
\begin{enumerate}
    ...
    \item Find the optimal step $\Delta \bfx^*$ by solving a linear set of equations \eqRef{eq:linear_syst_GN} \\
    This part does not change. As mentioned in \secRef{sec:LM}, alternatives to vanilla Gauss-Newton such as the Levenberg-Marquardt algorithm
...
are described as residuals weighted by their respective covariance matrices, representing the confidence we have on the sensor measurements. Factor Graphs can represent visually those problems and their connectivity is closely related to the sparsity of the linear problems appearing in the optimization algorithm.

Even though finding new solvers extending these ideas to new sets of problems, such as multi robot or multi hypothesis problems, is still an active research, 
formulating new, efficient, and generalizable measurement models is equally important. The next chapters each concentrate on a range of sensor modalities useful for legged robots:
