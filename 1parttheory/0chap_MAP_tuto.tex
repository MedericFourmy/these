...
property, define the so-called Lie group. Structural elements particularily useful are the existence of an identity element (that can be thought of as the origin of the manifold), the existence of inverse for each element, the adjoint linear map, and the possibility to interpolate between elements.

In this section, we will provide the minimal concepts regarding smooth manifolds and Lie groups that are useful to the roboticist. The section is organized as follows: first we will introduce smooth
manifolds and their properties, such as tangent spaces and retraction. Then we will show how those concept apply to Gauss-Newton optimization. Covariances and Jacobians on smooth manifold variables are then introduced. Finally, Lie groups in the robotics context will be shortly introduced.  
...
%
\begin{equation}
    \SO(3) = \left\{\Rot{}{} \in \text{GL}(n,\Reals) | \Rot{}{}\Rot{}{}^{\top}=\Rot{}{}^{\top}\Rot{}{}=I,~det(\Rot{}{})=1\right\}
\end{equation}
%
...
Due to the general non-commutativity of the grup operation, an additional $\oplus$ operator exists that retracts and composes vectors at the Lie algebra instead of
