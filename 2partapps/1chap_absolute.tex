...
the localization drift with absolute localization computed the from camera and/or the LIDAR with low bandwidth and higher 
...
and the other type called \emph{factors} represents constraints between variables, produced by the measurements.
%
...
% Although our implementation of the least-squares estimator is able to handle sensor calibration, we have not tried to calibrate the camera-to-IMU extrinsic parameters online.
...
%
We consider four datasets which are summarized in table \tabRef{tab:datasets}. They cover different tasks on which a consistent 
...
The biggest errors are obtained for the walking datasets where the two humps correspond to phases where the robot is turning on itself and sees landmarks that it will not 
see again later in the trajectory.
