...
\begin{figure}%[!ht]
  \centering
  \includegraphics[width=0.60\textwidth]{figures/cosyslam/solo_closer_small.png}
...
These approaches build metric maps that do not usually leverage the presence of known assets in the scene. In \cite{SalasMoreno2013SLAMSL}, 
...
As described in \secRef{sec:object_pose_est}, deep-learning-based object detection systems have now reached an accuracy that make them candidates for mobile robotics applications.
...
on empirical data, presented in \secRef{sec:cosypose_covariance}. We also detail practical tricks to circumvent outliers in the network output. 
...
thus it as the same factor graph. The \apriltag pose measurement model is replaced by the CosyPose measurement model.
...
for the experiments on the quadruped robot Solo \cite{grimminger2020open} as shown in Fig. \figRef{fig:solo_and_stairs}. 
...
\begin{figure}[h]
  \centering
  \includegraphics[width=0.8\textwidth]{figures/cosyslam/trajectory_circular.png}
...
to the result (Fig. \figRef{fig:traj_circular}). But we can notice that the MTE is also reduced (Table \tabRef{tab:tless}). Indeed, the motion model is more precise thanks 
...
We recorded video sequences including stairs with a camera fixed on a Solo robot (\ref{fig:map_stairs}). A stair has three discrete symmetries 
