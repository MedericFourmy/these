...
of strain cells as artificial, which provides a higher density haptic feed-back that strain-gauges, with demonstrated use for human robot interaction. 
A higher quality sense a touch may in provide richer information about the nature of the contacts between the robot and its environment, be it for locomotion
(slip detection, nature of the terrain etc.), or for manipulation (object surface analysis, 3D object position estimation).

Potential to add priors to vision models
