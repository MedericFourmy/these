...
% CNBC doc
% Interviewies: Fankhauser
% At the core, quadruped robots are mobility platforms that can be equipped with different payloads depending on the type of information that company want to gather
% ANYbotics CEO (Peter Fankhauser): can carry up to 15 kg of payload: visual cameras, thermal cameras, microphone, LED lights and gaz detection sensors -> example of leakage detection
% for fire prevention
% can be equipped with an arm to manipulate the env around them
% Price: 
%    - ANYbotics -> 150k$ including lidar and docking station
%    - Ghost robotics vision 60 -> 150k$
%    - Boston dynamics entry level Explorer Spot -> 75k$, for US national grid robot, just LIDAR+thermal cam +57k$
% National grid: improve the safety of operating conditions for workers. Very repeatable -> images from same angles/vantage
%   LIDAR for navigation + visual and thermal cameras for inspection of the substation -> detect assets hotspot for areas of concern, premature failure
%   Team up with IBM for cloud coomputing and AI for online detection
% Spot Other users:
% - Germany pharmacetical group Merck 
% - BP to read gauges, monitor corrosion and measure methane on oil rigs in the gulf of mexico 
% - Ford plant (Michigan) -> create a digital model of the plant 
% Anymal other users ()
% - Petronas (Maleysian oil company) -> inspect offshore platforms. manned and manual operations : 35% of annual operational epanditures.
% Personal training and equipment certification add to the cost. Oil rigs high risk areas 
% - Vale (Brazilian mining corporation) -> in the process of purchasing 1 to gather condition of equipment in one of its mines. Dangerous spaces with noise, dust and rotating parts
% - BASF (German chemical company) -> currently testing anymal to gather visual thermal and accoustic data  
% Developping an explosion proof version to avoid sparks from igniting explosive materials (no metal on metal contact, electronics flooded in nitrogen)
% Also (Both): construction sites and train yards (Anymal)

% Defense application: already autonomous used but people triggered by quadrupeds in particular (Black Mirror)
% 2021: NYC state police stop using spot because of public backlash
% Campaign to stop killer robots
% Boston Dynamics ethical principles -> will not allow use of its robot for weaponized autonomous systems
% Ghost Robotics (Philadelphia) -> 20+ customers, 90% US and allies. Airforce for security patrols in bases
%      October 2021: Ghost robotics quadruped equiped with a rifle manufactured by Sword International
% Farmers Insurance: use spot to assess damaged caused by natural disasters -> 1.7 billion $ in 2025 $

% Use still far from widespread: Spot sold a few hundred, ANYbotics < 100
