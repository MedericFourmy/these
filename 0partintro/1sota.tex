...
the CoM because of gesticulation \cite{wieber2000modelisation} induced angular momentum variations \cite{wieber2006holonomy}. Moreover, the wrench measurements are usually noisy
...
This fosters the use of more complex algorithms by fusing kinematic and external wrench measurements. Stephens \cite{stephens2011state} proposes to use the 
Linear Inverse Pendulum Model (LIPM) as process model to an EKF that also includes kinematic measurements. Model errors in the form of a CoM position measurement 
...
without introducing model biases as was done in Stephens \cite{stephens2011state}, instead relying on tuning filter covariances to alleviate their effects. 
...
One of the benefits of this model is that force measurements can be summarized by the Center of Pressure (CoP) 
...
sensor present on the Atlas robot. A centroidal estimator similar to that of Stephens \cite{stephens2011state} was implemented later implemented in the same team 
...
While previously mentioned works relied on approximated dynamical models, we must acknowledge that 
...
% In fact, 
an inaccurate process model must lead to a biased estimation. 
Later works tried instead to use exact dynamical models. In a first time, Xinjilefu \cite{xinjilefu2014dynamic} implemented an estimator using the whole-body dynamics
...
They formulated the estimator as a QP using this generic formulation to enforce inequality constraints on state variables. It is used to model joint limits and the positivity of normal ground reaction forces 
(the robot can only push on the ground).
Rotella \cite{rotella2015humanoid} proposes instead to use the Newton/Euler dynamics, which are exact an exact part of the whole-body dynamics. 
