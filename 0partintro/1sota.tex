...
The environment might be known through a previous mapping procedure like Structure from Motion \cite{triggs1999bundle} or mapped on the fly but a major limiting 
...
filter. This provided a faster localization with respect to the known global point-cloud map, which was built in advance. 
...
buffer of past belief state of the EKF.
...
can be computed intermittently. \cite{kolter2009stereo} adopts a mesh representation built from point-clouds aligned twice a seconds off-board using ICP initialized by proprioceptive odometry and filled with
...
tightly couple motion planning and terrain reconstruction. 
...
\cite{winkler2015planning} -> octomap based
...
As we have seen in section [], the largest part of the legged robotics state estimation literature consists in fusing sensor modalities using various 
implementations of the Bayesian and complementary filter. The core of those systems consists in a tightly or loosely coupled IMU/kinematics proprioceptive
...
rendering position and yaw observable.
...
such as drone navigation: Factor Graph optimization, also known as simply graph optimization or smoothing and mapping. While the Bayesian filter summarizes
the accumulated information about the current estimated state and its cross-correlations as a single probabilistic distribution, Factor Graph optimization
keeps a trajectory of past states which are regularly re-optimized in the least square sense, by finding the so called Maximum A Posterio over the trajectory
joint distribution. Many ad hoc efficient solvers such as\cite{grisetti2011g2o, dellaert2012factor, ila2017slam++, ceres-solver} have been developed 
over the years to leverage the specific sparsity of the SLAM problem.

While the first successful online visual SLAM algorithm was implemented using an EKF \cite{davison2007monoslam}, a breakthrough happened with the 
...
mapping in separate processes.

SLAM least square solvers: 

General purpose graph estimation frameworks: \cite{sola2021wolf}, \cite{blanco2019modular}, \cite{colosi2020plug}
