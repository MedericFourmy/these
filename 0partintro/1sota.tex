...
\cite{xinjilefu2016distributed,rotella2016imu} but it uses independent complementary filters for each IMU to recover their tilt (pitch and roll). This complementary
filter neglects linear accelerations of the IMU before gravity, filtering them as noise.
...
assumption and drop the use of the robot dynamical model. They design an observer to capable to estimate linear acceleration of each IMU of 
the network by fusing IMU measurements and the linear velocity measurements from the kinematics, which are propagated forward starting from the contacts. 
