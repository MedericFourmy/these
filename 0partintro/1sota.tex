...
These measurement sources convey wildly different types of information and arrive at various frequencies (eg. tens of Hertz for cameras, kilo Hertz for encoders). 
...
response filters, though this may introduce delays, or by fusing measurements using probabilistic filters. Bias are constant or slowly varying quantities affecting measures 
...
These few key terms explicated, we can now explore the legged estimation literature.
...
For point-feet robots (such as most quadrupeds), the problem is an instance of the orthogonal Procrustes problem \cite{eggert1997estimating}.
...
These measurement models are the base of all legged robots estimators. We will see in the next section how they can be used with other data sources, 
...
\begin{figure}
    ...
    orange elements correspond to the state variables affected by these residuals. Figures (\subref{fig:kin_point_matching}),(\subref{fig:kin_point_vel}),(\subref{fig:kin_point_direct}) are models
    for point feet robot, a model commonly used for quadrupeds. Figures (\subref{fig:kin_flat_matching}),(\subref{fig:kin_flat_vel}),(\subref{fig:kin_flat_direct}) are flat feet based models,
...
is the fact that adding extra weight at the very end of the legs can increase significantly the legs inertia, limiting the explosivity of possible movement. The 
extra electronics required may also be a limiting factor.  
...
Legged robots are highly nonlinear systems need contact forces between their end effector and the environment to realize stable locomotion. 
...
The contact wrench is not always directly accessible to estimator. For instance, the Atlas robot gauge sensors measure only the contact normal force
and horizontal moments. The exact underactuated dynamics cannot therefore be directly used. Some works therefore propose to use simplified models of the 
...
Stephens \cite{stephens2011state} proposes to use the LIPM as process model to an EKF 
...
One of the benefits of this model is that force measurements can be summarized by the Center of Pressure (CoP) (or ZMP \cite{sardain2004forces}).  
A centroidal estimator similar to that of Stephens \cite{stephens2011state} was implemented later implemented in the same team 
\cite{xinjilefu2015center} and made possible a fall detection algorithm that prevented a fall during CMU team DARPA robotic challenge finals. 
Similar EKF based estimators using respectively a LIPM and a fly-wheel process model were later implemented on a NAO robot \cite{piperakis2016non, piperakis2018nonlinear}. 
The robot used for the experiments being equipped with a single axis ground reaction sensor,
...
measurements and kinematic information, centroidal quantities can jointly be estimated with a joint flexibility at the ankle under external force disturbances. 
...
A generalized torque slack variable was introduce to represent the dynamical model errors. %External wrench at the feet is also a state variable as the measurement model on this quantity is only partial . 
...
\subsection{Underactuaded dynamics based estimators}
While previously mentioned works relied on approximate dynamical models, we must acknowledge that an inaccurate process model must lead to a biased estimation. 
...
Carpentier \cite{carpentier2016center} chose a different viewpoint by proposing a frequency analysis of the information sources used for centroidal estimation. The model
...
This algorithm estimated the same quantities as the previous work of the author \cite{bailly2019recursive} by solving a Maximum A Posteriori (MAP) problem on a sliding window of states sampled at IMU frequency, 
...
Any useful task undertaken by an autonomous robot involves some level of environment awareness, even for partially teleoperated robots \cite{koolen2016design}. 
For ground robots, this problem is most often tackled with exteroceptive sensors such as camera, depth camera and LIDARs.
...
autonomous exploration \cite{rouvcek2019darpa, kulkarni2021autonomous}... A intermediate case is the one of visual/LIDAR odometry in which a local representation of the environment is built in order to provide a precise odometry source \cite{scaramuzza2011visual} and is later discarded. 
...
robust enough so that a purely proprioceptive estimation provides enough feedback even in complex environments \cite{tan2018sim, lee2020learning}, having en estimate of 
...
exteroceptive localization and mapping at large. We will We will quickly review the most relevant works, first focusing on localization eg. for navigation then 
...
shape of the environment enables for reactive contact planning. For instance, the guide path implemented in contact planner \cite{tonneau2018efficient} uses the complete 3D structure of the environment
...
In order handle local loopy trajectories better than Kintinuous, Elastic Fusion \cite{whelan2016elasticfusion}, which is a surfel based dense depth vision system, 
...
dense reconstruction is accurate up to 2cm, which is enough for motion planning. However, the map in this case was only used for localization.

% CONCLU
Online terrain reconstruction extends the capabilities of legged robots to uneven surfaces. It is however not central to the problematic of this thesis and is considered as
...
such as drone navigation: \textit{factor graph optimization}, also known as simply \textit{graph optimization} or \textit{online batch estimation}. 
...
Maximum A Posteriori over the trajectory joint distribution. The terms smoothing and mapping or sliding window estimator are also used when a limited window of past state are kept 
...
mapping in separate processes. Mapping was handled by the bundle adjustment of sparse features, a method that was previously reserved for offline SfM 
pipelines \cite{triggs1999bundle}. PTAM enabled Augmented Reality applications in limited spaces. 
A great wealth of papers following both methodologies were subsequently published. A thorough comparison 
of the two approaches \cite{strasdat2012visual} concluded that the graph optimization was superior in that it had the greatest precision to 
...
\subsection{Theoretical intro to factor graphs}
...
